...
//
// The core abstraction is a Step, which represents a single step in a
// computation that uses continuation-passing style - each step returns the
// next step to execute, or nil to terminate the pipeline.
//
//	type Step interface {
//		Run(context.Context) Step
//	}
//
// This eliminates the need for complex builder patterns and ID-based step
// lookup found in the step package, making composition more direct and
// intuitive.
//
// # Basic Usage
//
// Create steps using Step for operations with context transformation:
//
//	step := state.Do(func(ctx context.Context) context.Context {
//		fmt.Println("processing...")
//		return ctx
//	})
//
// Compose steps using Sequence for sequential execution:
//
...
//
// # Comparison with Step Package
//
// The state package simplifies the patterns from the step package:
//
//	// step package (old)
//	hasSecretHandler := step.Chain(
//		ensureMetadataBuilder,
//		ensureCredsBuilder,
//	).Step("hasSecret")
//
//	mainHandler := step.Chain(
//		setFinalizerBuilder,
...
// # Benefits
//
// - No ID management: Direct references eliminate step IDs and lookup
// - Simpler composition: Steps compose naturally without builder patterns
...
// - Reusability: Steps easily reused across different pipelines
// - Testability: Individual steps and compositions easy to test
//
